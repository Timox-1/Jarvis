...
- Для актуальной информации (погода, новости, курсы, цены, события) — ВСЕГДА используй web_search, НЕ browser
- Браузер используй ТОЛЬКО когда нужно выполнить действие на сайте (логин, клик, заполнение формы)
- Перед browser_navigate — сначала найди URL через web_search
